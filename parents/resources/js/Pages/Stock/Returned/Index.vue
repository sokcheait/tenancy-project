        ...
        <div class="w-full p-4 relative">
            <div class="bg-blue-500 dark:bg-gray-800 shadow-lg rounded-md flex items-center justify-between p-3 border-b-4 border-blue-600 dark:border-gray-600 text-white font-medium group">
                ...
                <div class="text-right">
                    <Link :href="route('returned.create')" class="text-gray-200 dark:text-white">
                        <Button
                            label="Create"
                            variant="primary"
                        />
            ...
            <div class="dark:bg-gray-800 shadow-lg rounded-md p-2 dark:border-gray-600 font-medium relative">  
                <Table :resource="return_list" class="dark:border-gray-600 font-medium">
                    <template #cell(id)="{ item: re }">
                        {{ re.id }}
                    </template>
                    <template #cell(suppliers)="{ item: re }">
                        {{ re?.suppliers.name }}
                    </template>
                    <template #cell(actions)="{ item: re }">
                            ...
                            >
                                ...
                                <DropdownOption>
                                    <div v-if="is_superAdmin('super-admin')" class="flex w-full">
                                        <Link :href="route('purchase-order.show',re.id)" class="w-full">
