...
export default {
    ...
    methods: {
        back() {
            this.$inertia.replace('/returned')
        },
        submitReturn() {     
            this.form.amount= this.sub_total;
            this.form.post(route('returned.store',{
                data:this.data_item
                }), {
                preserveScroll: true,
                onSuccess: () => {
                    this.toast.success("Create returned successfully");
                },
                onError: () => {
                    this.toast.error("Create returned Errors");
                },
            });
        },
        listItem(key) {
...
<template>
    <AppLayout title="returned-create">
            ...
            <div class="bg-blue-500 dark:bg-gray-800 shadow-lg rounded-md flex items-center justify-between p-3 border-b-4 border-blue-600 dark:border-gray-600 text-white font-medium group">
                <div class="flex justify-center items-center h-4 transition-all duration-300 transform px-4">
                    ...
                    <span class="ml-3 mt-1">
                        <Link :href="route('returned.index')" class="text-gray-200 dark:text-white">
                            Returned
                        </Link>
                    </span>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-lg rounded-md p-2 dark:border-gray-600 dark:text-white font-medium group">
                <form @submit.prevent="submit">
                    <div class="flex flex-wrap">
                        <div class="w-full">
                            <h3 class="text-gray-600 font-bold block p-2 mt-4 bg-gray-200 dark:bg-gray-800 dark:text-white border rounded">Returned</h3>
                        </div>
                        <input-text v-model="form.return_code" 
                                inputLable="Returned Code"
                                :disabled="true"
                        />
