        ...
        <div class="w-full p-4">
            <div class="bg-blue-500 dark:bg-gray-800 shadow-lg rounded-md flex items-center justify-between p-3 border-b-4 border-blue-600 dark:border-gray-600 text-white font-medium group">
                ...
                <div class="text-right">
                    <Link v-if="can('categories.create') || is_superAdmin('super-admin')" :href="route('categories.create')" class="text-gray-200 dark:text-white">
                        <Button
            ...
            <div class="dark:bg-gray-800 shadow-lg rounded-md p-2 dark:border-gray-600 font-medium group">  
                <Table :resource="categories" class="dark:border-gray-600 font-medium group">
                    <template #cell(is_active)="{ item: category }">
                        <div class="flex ml-6 focus:ring-0 focus:ring-offset-0 focus:border-primary-600">
                            <span class="relative inline-flex">
                                <span v-if="category.is_active" class="flex absolute h-4 w-4 top-0 right-0 -mt-1 -mr-1 z-10">
                    ...
                    <template #cell(actions)="{ item: category }">
                        <div class="flex">
                            <span v-if="can('categories.edit') || is_superAdmin('super-admin')">
                                <Link :href="route('categories.edit',category.id)">
                                    <PencilIcon class="text-blue-600 w-5 h-5"/>
                                </Link>
                            </span>
                            <span v-if="can('categories.destroy') || is_superAdmin('super-admin')"><TrashIcon class="text-rose-500 mx-2 cursor-pointer w-6 h-5" @click="deleteCategory(category.id)"/></span>
                        </div>
                    </template>
                </Table>
            </div>
