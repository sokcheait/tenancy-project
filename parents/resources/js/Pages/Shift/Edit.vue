...
export default {
    ...
    data() {
        return {
            ...
            form :useForm({
                ...
                time_from: this.time,
                time_to: this.time,
                weekend_definition: JSON.parse(this.shift?.weekend_definition),
                shift_allowance: this.shift?.shift_allowance,
            }),
        }
    },
    created() {
        if(this.shift?.time_from){
            const time_from = this.formatTime(this.shift?.time_from)
            this.form.time_from = time_from;
        }
        if(this.shift?.time_to){
            const time_to = this.formatTime(this.shift?.time_to)
            this.form.time_to = time_to;
        }
    },
    watch: {
    ...
    methods: {
        ...
        updateShift() {
            console.log(this.form.time_from);
            this.form.put(route('shift.update',this.shift.id), {
                preserveScroll: true,
                onSuccess: () => {
                    this.toast.success("Update shift successfully", {
                    });
                },
                onError: () => {
                    this.toast.errors("Update shift Errors", {
                    });
                },
            });
        },
        formatTime(key) {
            let time = key.split(':');
            let result = {"hours": time[0], "minutes": time[1]};
            return result;
        }
    }
        ...
        <div class="w-full p-4">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-lg rounded-md p-2 dark:border-gray-600 text-white font-medium group">    
                <form @submit.prevent="updateShift">
                    ...
                    >
                        <div class="flex">
                            <div class="w-1/2 mr-4 text-black">
