    ...
    <div class="relative flex flex-col flex-1 min-h-0 pt-0 bg-white border-r border-gray-200 dark:bg-gray-800 dark:border-gray-700">
        <div class="flex flex-col flex-1 pt-5 pb-4 overflow-y-auto">
            <div class="flex-1 px-3 space-y-1 bg-white">
                <ul v-for="nav_menus in menu" :key="nav_menus" class="pb-2 space-y-2">
                    <li v-if="nav_menus.header==null">
                        <Link :href="route(nav_menus.link)" :active="route().current(nav_menus.link)"
                            :class="{'bg-gray-100':route().current(nav_menus.link)}"
                            class ="flex items-center py-2 text-base text-gray-800 rounded-lg hover:bg-gray-100 group dark:text-gray-200 dark:hover:bg-gray-700">
                                <span class="w-6 h-6 mx-2 text-gray-500 transition duration-75 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white">
                                    <component :is="heroIcons[nav_menus.icon]" class='h-6 w-6' :class="{'text-purple-500': route().current(nav_menus.link)}" />
                                </span>
                                <span :class="{'text-purple-600': route().current(nav_menus.link)}">
                                    {{ nav_menus.lable }}
                                </span>
                        </Link>
                    </li>
                    <li v-else>
                        <button @click="openClose" type="button" class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700 transition">
                            ...
                            <span class="flex-1 ml-3 text-left whitespace-nowrap" sidebar-toggle-item>{{ nav_menus.header.title }}</span>
                            <ChevronRightIcon class="w-5 h-5 collapse-enter-active" v-if="isShow==false" />
                            <ChevronDownIcon class="w-5 h-5 collapse-enter-active" v-if="isShow==true" />
                        </button>
                        <Transition name="collapse">
                            <div class="collapsableDiv" v-if="isShow">
                                <ul id="" class="py-2 space-y-2">
                                    <li v-for="submenu in nav_menus.submenu" :key="submenu">
                                        <Link :href="route(submenu.link)" :active="route().current(submenu.link)"
                                            :class="{'bg-gray-100':route().current(submenu.link)}"
                                            class ="flex items-center py-2 pl-4 text-base text-gray-800 rounded-lg hover:bg-gray-100 group dark:text-gray-200 dark:hover:bg-gray-700">
                                                <span class="w-6 h-6 mx-2 text-gray-500 transition duration-75 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white">
                                                    <component :is="heroIcons[submenu.icon]" class='h-5 w-5' :class="{'text-purple-500': route().current(submenu.link)}" />
                                                </span>
                                                <span :class="{'text-purple-600': route().current(submenu.link)}">
                                                    {{ submenu.lable }}
                                                </span>
                                        </Link>
                                    </li>
